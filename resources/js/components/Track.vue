<template>
    <tr class="border-b border-gray-800 hover:bg-gray-700" ref="track">
        ...
        <td class="p-2 w-10">
            ...
            <button v-else @click.prevent="setSong(song)">
        ...
        <td class="p-2" v-if="displayArtist">
            <router-link tag="button" :to="'/artists/' + song.artist.id" class="focus:outline-none hover:underline">
                {{ song.artist.name }}
            </router-link>
        </td>
        <td class="p-2" v-if="displayAlbum">
            <router-link tag="button" :to="'/albums/' + song.album.id" class="focus:outline-none hover:underline">
                {{ song.album.name }}
            </router-link>
        </td>
